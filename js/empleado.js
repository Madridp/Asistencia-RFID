...
var formulario_agregarEmpleado = document.getElementById("formulario_agregarEmpleado");
...
if ( document.getElementById('guardar') ){
	document.getElementById('guardar').addEventListener("click",(e)=>{
		...
		if ( puesto == ""){
			decir("¨No ha indicado el puesto")
			return;
		}

		if ( telefono == ""){
			decir("¨No ha indicado el telefono")
			return;
		}

		if ( id_usuario == ""){
			decir("¨No ha indicado el id del usuario")
			return;
		}
		var xhr = new XMLHttpRequest();
		xhr.open('POST', 'empleado_guardar.php', true);
		xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		xhr.onload = function () {
			console.log(this.responseText);

			if ( this.responseText ==  1 ){
				decir("Empleado agregado correctamente");
		...
		xhr.send("nombre=" + nombre + "&id_puesto=" + puesto + "&telefono=" + telefono + "&id_usuario=" + id_usuario);
	});
}
